...
namespace NodeTools.Debugger
{
    ...
    internal class ProvideDebugEngineAttribute : RegistrationAttribute
    {
        ...
        {
            Key engineKey = context.CreateKey("AD7Metrics\\Engine\\" + _id);
            ...
            using (Key incompatKey = engineKey.CreateSubkey("IncompatibleList"))
            {
            ...
            Key clsidKey = context.CreateKey("CLSID");
            Key clsidGuidKey = clsidKey.CreateSubkey(_debugEngine.GUID.ToString("B"));
            ...
            using (Key exceptionAssistantKey = context.CreateKey("ExceptionAssistant\\KnownEngines\\" + _id))
            {
