...
namespace NodeTools.Debugger
{
    ...
    {
        private readonly string _eeGuid;
        private readonly string _engineGuid;
        private readonly string _languageGuid, _languageName;
        ...
        {
            Key langSvcKey = context.CreateKey("Languages\\Language Services\\" + _languageName + "\\Debugger Languages\\" + _languageGuid);
            langSvcKey.SetValue("", _languageName);
            // 994... is the vendor ID (Microsoft)
            Key eeKey = context.CreateKey("AD7Metrics\\ExpressionEvaluator\\" + _languageGuid + "\\{994B45C4-E6E9-11D2-903F-00C04FA302A1}");
            eeKey.SetValue("Language", _languageName);
            eeKey.SetValue("Name", _languageName);
            eeKey.SetValue("CLSID", _eeGuid);

            Key engineKey = eeKey.CreateSubkey("Engine");
