...
namespace Console.ConsoleWindow
{
    ...
    /// </summary>
    class HostInfo : ObjectWithFactory<ConsoleWindow>, IDisposable
    {
        Lazy<IHostProvider, IHostMetadata> HostProvider { get; set; }

        public HostInfo(ConsoleWindow factory, Lazy<IHostProvider, IHostMetadata> hostProvider)
            : base(factory)
        {
            ...
            get
            {
                if (_wpfConsole == null)
                {
                    _wpfConsole = Factory.WpfConsoleService.CreateConsole(
                        Factory.ServiceProvider, ConsoleWindow.ContentType, HostName);
