...
namespace ProjectSystem.Infrastructure
{
    public static class CommonUtils
    {
        ...
        public static string NormalizePath(string path)
        {
            ...
            Uri uri = MakeUri(path, false, UriKind.RelativeOrAbsolute);
        ...
        public static string NormalizeDirectoryPath(string path)
        {
            ...
            Uri uri = MakeUri(path, true, UriKind.RelativeOrAbsolute);
            if (uri.IsAbsoluteUri)
            {
                if (uri.IsFile)
                {
        ...
        public static bool IsSubpathOf(string root, string path)
        {
            ...
            Uri uriRoot = MakeUri(root, true, UriKind.Absolute, "root");
            Uri uriPath = MakeUri(path, false, UriKind.Absolute, "path");
            ...
            Uri uriDirectoryPath = MakeUri(path, true, UriKind.Absolute, "path");
        ...
        public static string GetAbsoluteDirectoryPath(string root, string relativePath)
        {
            ...
            Uri relUri = MakeUri(relativePath, true, UriKind.RelativeOrAbsolute, "relativePath");
            Uri absUri;
            ...
            {
                Uri rootUri = MakeUri(root, true, UriKind.Absolute, "root");
                try
                {
        ...
        {
            Uri rootUri = MakeUri(root, true, UriKind.Absolute, "root");
            Uri relUri = MakeUri(relativePath, false, UriKind.RelativeOrAbsolute, "relativePath");
        ...
        /// </summary>
        /// <exception cref="ArgumentException">
        ///     Either parameter was an invalid or a
        ///     relative path.
        /// </exception>
        public static string GetRelativeDirectoryPath(string fromDirectory, string toDirectory)
        {
            Uri fromUri = MakeUri(fromDirectory, true, UriKind.Absolute, "fromDirectory");
            Uri toUri = MakeUri(toDirectory, true, UriKind.Absolute, "toDirectory");

            string relPath;
            char sep = toUri.IsFile ? Path.DirectorySeparatorChar : Path.AltDirectorySeparatorChar;
            ...
            try
            {
                ...
                else
                {
                    ...
                    string rootedPath = TrimUpPaths(relPath);
        ...
        {
            Uri fromUri = MakeUri(fromDirectory, true, UriKind.Absolute, "fromDirectory");
            Uri toUri = MakeUri(toFile, false, UriKind.Absolute, "toFile");
        ...
        {
            string relativePath = GetRelativeDirectoryPath(basePath, path);
