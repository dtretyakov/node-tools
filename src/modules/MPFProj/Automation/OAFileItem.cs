                ...
                return UIThread.DoOnUIThread(delegate
                    {
                        ...
                        using (var scope = new AutomationScope(Node.ProjectMgr.Site))
                        {
                            ...
                            VsShellUtilities.IsDocumentOpen(Node.ProjectMgr.Site, Node.Url, VSConstants.LOGVIEWID_Any, out hier, out itemid, out windowFrame);
            ...
            return UIThread.DoOnUIThread(delegate
                {
                    ...
                    using (var scope = new AutomationScope(Node.ProjectMgr.Site))
                    {
                        try
                        {
                            ...
                            ErrorHandler.ThrowOnFailure(rdt.FindAndLockDocument((uint) _VSRDTFLAGS.RDT_NoLock, Node.Url, out ivsHierarchy, out itemid, out docData, out docCookie));

                            // Open the file using the IVsProject3 interface
                            ErrorHandler.ThrowOnFailure(Node.ProjectMgr.OpenItem(Node.ID, ref logicalViewGuid, docData, out windowFrame));
                        }
                        finally
                        {
            ...
            return UIThread.DoOnUIThread(delegate
                {
                    ...
                    using (var scope = new AutomationScope(Node.ProjectMgr.Site))
                    {
                        ...
                        isOpen = VsShellUtilities.IsDocumentOpen(Node.ProjectMgr.Site, Node.Url, logicalViewGuid, out hier, out itemid, out windowFrame);
                    }
            ...
            UIThread.DoOnUIThread(delegate
                {
                    ...
                    using (var scope = new AutomationScope(Node.ProjectMgr.Site))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                ErrorHandler.ThrowOnFailure(Node.ProjectMgr.SaveItem(VSSAVEFLAGS.VSSAVE_SilentSave, url, Node.ID, docData, out canceled));
                            }
                            else
                            {
                                ...
                                if (!isCalledFromSaveAs)
                                {
                                    ...
                                    ErrorHandler.ThrowOnFailure(Node.ProjectMgr.SaveItem(VSSAVEFLAGS.VSSAVE_SilentSave, fullPath, Node.ID, docData, out canceled));
                                }
                                else
                                {
                                    ErrorHandler.ThrowOnFailure(Node.ProjectMgr.SaveItem(VSSAVEFLAGS.VSSAVE_SilentSave, fullPath, Node.ID, docData, out canceled));
