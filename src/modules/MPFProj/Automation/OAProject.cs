...
namespace Microsoft.VisualStudio.Project.Automation
{
    ...
    public class OAProject : EnvDTE.Project, ISupportVSProperties
    {
        ...
        {
            get { return "{262852C6-CD72-467D-83FE-5EEB1973A190}"; }
        }
                    ...
                    return UIThread.DoOnUIThread(delegate
                        {
                            ...
                            ErrorHandler.ThrowOnFailure(solution.GetUniqueNameOfProject(project.InteropSafeIVsHierarchy, out uniqueName));
                            return uniqueName;
                        });
                }
            }
                ...
                return UIThread.DoOnUIThread(delegate
                    {
                        if (configurationManager == null)
                        {
                            ...
                            ErrorHandler.ThrowOnFailure(extensibility.GetConfigMgr(project.InteropSafeIVsHierarchy, VSConstants.VSITEMID_ROOT, out configurationManagerAsObject));
            ...
            UIThread.DoOnUIThread(delegate
                {
                    using (var scope = new AutomationScope(project.Site))
                    {
                        ...
                        if (!isCalledFromSaveAs && string.IsNullOrEmpty(fileName))
                        {
                            ...
                            IntPtr unkData;
                            uint cookie;
                            ErrorHandler.ThrowOnFailure(rdt.FindAndLockDocument((uint) _VSRDTFLAGS.RDT_NoLock, project.Url, out hier,
